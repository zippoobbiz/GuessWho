...
public class GuessWho {
	...
	public static void usage() {
		System.err
				.println(progName
						+ ": [-l <filename to write game logs to>] [game configuration filename] [chosen persons filename] [type of player1] [type of player2]");
	...
	public static void main(String[] args) {
		...
		// -l <inputFilename> specifies the file to write game log information
		// to.
		if (options.has("l")) {
			bLog = true;
		...
		try {
			...
			BufferedReader assignedReader = new BufferedReader(new FileReader(
					assignedFilename));
			...
			if ((line = assignedReader.readLine()) != null) {
				...
				if (fields.length != 2) {
					throw new IOException(assignedFilename
							+ ": Misformed field line: " + line);
				}
			...
			// Each player will load the game configuration and store it
			...
			case "random":
				player1 = new RandomGuessPlayer(gameFilename,
						player1AssignedName);
				break;
			// Task C (binary search based guessing player)
			case "binary":
				player1 = new BinaryGuessPlayer(gameFilename,
						player1AssignedName);
				break;
			// Bonus Task (your customised guessing player)
			case "custom":
				player1 = new CustomGuessPlayer(gameFilename,
						player1AssignedName);
				break;
			default:
				System.err.println("Uknown player 1 type option: "
						+ player1Type);
				usage();
			}
			...
			// Task B (random guessing player)
			case "random":
				player2 = new RandomGuessPlayer(gameFilename,
						player2AssignedName);
				break;
			// Task C (binary search based guessing player)
			case "binary":
				player2 = new BinaryGuessPlayer(gameFilename,
						player2AssignedName);
				break;
			// Bonus Task (your customised guessing player)
			case "custom":
				player2 = new CustomGuessPlayer(gameFilename,
						player2AssignedName);
				break;
			default:
				System.err.println("Uknown player 2 type option: "
						+ player2Type);
				usage();
			}
			...
			// Note if you do not have any implementation, receiveAnswer() will
			// always
			// return true (to prevent infinite loop), so game ends after 1
			// round
			// as tie.
			int round = 1;
			while (!player1Finished && !player2Finished) {
				...
				// If player 1 made a person guess and it was correct,
				...
				// If player 2 made a person guess and it was correct,
				// player2Finished should be true;
				// otherwise be false
...
class MoveLog {
	...
	/**
	 * Constructor, that sets output to be written to a log file as well as
	 * System.out.
	 *
	 * @param outFilename
	 *            The name of the file to write game log output to.
	 * @throws IOException
	 *             When there are IO issues from openning file.
	 */
